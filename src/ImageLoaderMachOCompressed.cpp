...
{
	// 申请空间，初始化参数，调用构造函数，需要注意的是构造函数生成了ImageLoaderMachOCompressed对象，同时在结尾附加了 SegmentMachO object的数组，用来保存segment的偏移，方便后续快速定位位置
	ImageLoaderMachOCompressed* image = ImageLoaderMachOCompressed::instantiateStart(mh, path, segCount, libCount);

	// set slide for PIE programs ， 记录偏移，后续用于映射实际的地址
	...
	// 初始化一些参数，比如get real fMachOData, fLinkEditBase, and fSlide
	image->instantiateFinish(context);
	image->setMapped(context); // 标识状态为mapped
...
// construct ImageLoaderMachOCompressed using "placement new" with SegmentMachO objects array at end 在构造函数后面还有一个 SegmentMachO的数组，用于保存segment的偏移，后面会利用这个来快速加载segment
																			...
																			unsigned int segCount, unsigned int libCount)
{
	...
	if ( allocatedSpace == NULL )
		throw "malloc failed";
	// 在ImageLoaderMachOCompressed对象后面，申请segOffsets数组，用于保存每个seg的具体偏移量
...
void ImageLoaderMachOCompressed::rebase(const LinkContext& context, uintptr_t slide)
{
	...
	try {
		...
		bool done = false;
		while ( !done && (p < end) ) {
			uint8_t immediate = *p & REBASE_IMMEDIATE_MASK; // 0x0F
			// 计算opcode
			uint8_t opcode = *p & REBASE_OPCODE_MASK; //  REBASE_OPCODE_MASK : 0xF0
			++p;
			switch (opcode) {
				...
				case REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
					...
					// 解码ULEB模式 https://en.wikipedia.org/wiki/LEB128
...
//bindAt(const LinkContext& context, ImageLoaderMachOCompressed* image,
//											 uintptr_t addr, uint8_t type, const char* symbolName,
//											 uint8_t symbolFlags, intptr_t addend, long libraryOrdinal,
//											 ExtraBindData *extraBindData,
//											 const char* msg, LastLookup* last, bool runResolver)
