...
void link(ImageLoader* image, bool forceLazysBound, bool neverUnload, const ImageLoader::RPathChain& loaderRPaths, unsigned cacheIndex)
{
	...
	// we detect root images as those not linked in yet // 这个是未来用于通知register_func_for_add_image function的
	if ( !image->isLinked() )
		addRootImage(image);
		...
		uintptr_t* startGlue)
{
	...
	// 获取主程序的hash，额外的参数，可以在程序的main函数里面通过argv来打印出来（argv的末尾 + 1的位置），例如executable_cdhash=fc4fa3da6c11cb38747ea3fa1666f8f96564de24
	...
	//主程序运行的路径，例如executable_path=/var/containers/Bundle/Application/7193E33C-1690-4E6C-B3B4-CC1D206E58C2/YourApp.app/YourApp
	sExecPath = _simple_getenv(apple, "executable_path");

	// ...

	// Remember short name of process for later logging
	// sExecShortName = YourApp
	sExecShortName = ::strrchr(sExecPath, '/');
	// ...
	
	// 配置进程限制，参考https://www.offensive-security.com/offsec/amfi-syscall/ 主要是调用AMFI来配置一些系统参数，比如是否容许注入动态库
	// 真机调试了下，主要是调用 (amfi_check_dyld_policy_self(amfiInputFlags, &amfiOutputFlags) == 0)
	// amfiOutputFlags返回了 0x40,对应 AMFI_DYLD_OUTPUT_ALLOW_LIBRARY_INTERPOSING ，所以Debug下是容许注入动态库的，这个可以从初始打印的入参也可以推测到
	// 但是线上release包并不确定，需要进一步定位
	...
	if ( sEnv.DYLD_PRINT_OPTS )
		// 打印入参，即在scheme里面Argument Passed On Launch里面配置，scheme的环境变量配置：DYLD_PRINT_OPTS，打印入参
		printOptions(argv);
	if ( sEnv.DYLD_PRINT_ENV )
		// 打印环境变量，scheme的环境变量配置： DYLD_PRINT_ENV
    ...
    // 获取临时文件路径，通过环境变量 比如： TMPDIR=/private/var/mobile/Containers/Data/Application/F9878FE5-6911-4345-ADED-E5EF67F7FF57/tmp
	...
	if ( sJustBuildClosure )
		sClosureMode = ClosureMode::On; // 默认会进来
	// 获取cpu类型的数据
	getHostInfo(mainExecutableMH, mainExecutableSlide);

	// load shared cache 加载共享的动态库，里面集成了常用的动态库,并打包在了一起，可以配置DYLD_PRINT_SEGMENTS环境变量来查看结果：
	// 共享库可以在这里找到 (/System/Library/Caches/com.apple.dyld/dyld_shared_cache_arm64e).
	if ( gLinkContext.sharedRegionMode != ImageLoader::kDontUseSharedRegion ) {
		mapSharedCache();
		// => mapCacheSystemWide
	}

	// 处理closure，如果加载成功就直接返回了
    // ...
	...
	// 各种回调初始化，比如 image 加载的回调 _dyld_register_func_for_add_image
	...
	try {
		...
		// 加载主程序所在的image， 最终调用 ImageLoaderMachOCompressed::instantiateMainExecutable，将主文件 MachO 做一些初步的解析为ImageLoaderMachO
		...
		// get path of dyld itself 获取dyld库的path，利用__dso_handle这个函数地址获取
		...
		// load any inserted libraries 预先加载DYLD_INSERT_LIBRARIES的lib到内存里，加载image最终调用addImages，image对象保存在了AllImages里
		...
		// 所以这里判断sAllImages的数量就知道加载了几个Insertedlib
		...
		if ( mainExcutableAlreadyRebased ) {
			...
			// work around that by rebasing by inverse amount , 查看 ImageLoaderMachOCompressed::rebase
			sMainExecutable->rebase(gLinkContext, -mainExecutableSlide);
		}
#endif
		// link 主工程 最终调用ImageLoader::link
		link(sMainExecutable, sEnv.DYLD_BIND_AT_LAUNCH, true, ImageLoader::RPathChain(NULL, NULL), -1);
		...
		// link any inserted libraries 		// 执行动态注入的那些库的link
